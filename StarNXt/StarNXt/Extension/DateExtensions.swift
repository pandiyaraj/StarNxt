...
//    }
    ...
    public var checkIsMarketOpen : Bool {
    ...
    public var isMarketTime:Bool {
       
        let marketOpenTime = self.dateAt(hours: 9, minutes: 15)
        let marketClosedTime = self.dateAt(hours: 15, minutes: 30)
        if self >= marketOpenTime &&
            self <= marketClosedTime
        {
            return true
        }
        return false
    ...
    func createHoliday(day : Int, month : Int) -> Date
    {
        let calendar = NSCalendar(calendarIdentifier: NSCalendar.Identifier.gregorian)!
    ...
    public func holidayLists()-> [Date] {
        
        let aug15 = self.createHoliday(day: 15, month: 08)
        let aug25 = self.createHoliday(day: 25, month: 08)
        let oct2  = self.createHoliday(day: 02, month: 10)
        let oct19 = self.createHoliday(day: 19, month: 10)
        let oct20 = self.createHoliday(day: 20, month: 10)
        let dec25 = self.createHoliday(day: 25, month: 12)
        
        return [aug15,aug25,oct2,oct19,oct20,dec25]
    ...
    public var isTodayHoliday:Bool{
        var isHolday : Bool = false
        for currentDate in holidayLists(){
